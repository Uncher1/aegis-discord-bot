...
function parsePerms(
  ...
  for (const n of names) {
    ...
    if (flag === undefined) {
      return {
        ok: false,
        error: `Permission inconnue dans ${context}: "${n}". Utilise un nom de PermissionFlagsBits (ex: ViewChannel, SendMessages, Connect, ManageChannels...).`,
      };
    }
    bits |= flag;
  }
